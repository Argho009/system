-- Update internal email suffix to @college
-- And migration to update existing users with WHERE clauses to satisfy safe-update checks

-- 1. Update existing auth.users and auth.identities
-- Using WHERE email LIKE '%@college.edu' to target only the old format
UPDATE auth.users SET email = REPLACE(email, '@college.edu', '@college') WHERE email LIKE '%@college.edu';
UPDATE auth.identities SET identity_data = jsonb_set(identity_data, '{email}', 
    to_jsonb(REPLACE(identity_data->>'email', '@college.edu', '@college'))
) WHERE identity_data->>'email' LIKE '%@college.edu';

-- 2. Update the creation helper
...
BEGIN
    ...
    ELSE
        UPDATE auth.identities SET identity_data = jsonb_build_object('sub', target_id, 'email', p_user.college_id || '@college')
