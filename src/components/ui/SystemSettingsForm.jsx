    ...
    try {
      ...
      if (data) {
        ...
        setConfig({
          ...
          lectures_per_day: parseInt(configObj.lectures_per_day, 10) || 8,
          working_days_per_week: parseInt(configObj.working_days_per_week, 10) || 6,
        });
      }
    } catch {
  ...
  const handleSave = async () => {
    ...
    try {
      for (const u of updates) {
        await api.putSystemConfig({ ...u, updated_by: user?.id });
      }
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) => setConfig({ ...config, lectures_per_day: parseInt(e.target.value, 10) })}
            />
          </div>
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) => setConfig({ ...config, working_days_per_week: parseInt(e.target.value, 10) })}
            />
          </div>
        </div>
