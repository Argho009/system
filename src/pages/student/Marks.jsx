...
export const StudentMarks = () => {
  ...
  async function fetchData() {
    ...
    const [ctRes, esRes] = await Promise.all([
      supabase.from('ct_marks').select('*, subjects(code, name)').eq('student_id', stu.id).order('created_at', { ascending: false }),
      supabase.from('endsem_marks').select('*, subjects(code, name)').eq('student_id', stu.id).eq('sem', stu.sem),
    ]);
          ...
          <div className="space-y-4">
            {Object.entries(ctBySubject).map(([subId, sub]) => (
              <div key={subId} className="bg-white border border-slate-200 rounded-lg p-4">
                <p className="font-semibold text-slate-800 mb-3">{sub.code} — {sub.name}</p>
                    ...
                    return (
                      <div key={t.id} className="bg-slate-50 rounded-lg p-3 text-center border border-slate-200">
                        ...
                        <p className="text-2xl font-bold text-slate-900 mt-1">
                          {t.marks_obtained ?? '—'}<span className="text-sm font-normal text-slate-400">/{t.max_marks}</span>
                        </p>
