...
export const Notices = () => {
  ...
  const handlePost = async (e) => {
    ...
    try {
      await api.createNotice({
        ...
        sem: form.sem ? parseInt(form.sem, 10) : null,
  ...
  const togglePin = async (id, pinned) => {
    try {
      await api.patchNotice(id, { is_pinned: !pinned });
      fetchNotices();
    } catch {
      /* ignore */
    }
  ...
  const handleDelete = async () => {
    try {
      await api.patchNotice(deleteTarget, { is_active: false });
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Announcements for assignments, labs, and general notices.
          </p>
        </div>
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <Badge variant={TYPE_COLORS[n.type] || 'default'}>{n.type}</Badge>
