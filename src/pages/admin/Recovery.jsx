...
export const EmergencyRecovery = () => {
  ...
  const fetchDeletedUsers = async () => {
    setLoading(true);
    try {
      const data = await api.getDeletedUsers();
  ...
  const handleRestore = async (user) => {
    ...
    try {
      await api.updateUser(user.id, { deleted_at: null, is_active: true });
      toast.success('User restored successfully');
      fetchDeletedUsers();
    } catch (err) {
      toast.error('Restoration failed: ' + (err.message || err));
    }
  };
  ...
  const handlePermanentDelete = async (user) => {
    if (
      !confirm(
        `WARNING: This will PERMANENTLY delete ${user.name} from the database. This action CANNOT be undone. Proceed?`
      )
    )
      return;
    ...
    try {
      ...
      toast.error('Permanent deletion failed: ' + (err.message || err));
    }
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-red-100 text-sm opacity-90">
                Restore recently deleted accounts or perform permanent erasure.
              </p>
            </div>
          </div>
          <div className="flex bg-white/10 backdrop-blur-md rounded-md p-1 items-center border border-white/20">
            ...
            <div className="px-4 py-2 text-center">
              ...
              <span className="text-[10px] uppercase font-bold tracking-widest opacity-70 mt-1 block">
                Log
              </span>
            </div>
          </div>
      ...
      <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg flex gap-3 italic">
        ...
        <p className="text-xs text-amber-700 leading-relaxed">
          <strong>Note on Recovery:</strong> Restoring a user will immediately reactivate their login and
          return them to the active user lists. Permanent erasure is irreversible and will lose all
          associated metadata for that user ID.
        </p>
      </div>
    </div>
  );
