...
export const SemTransition = () => {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      setBranches(bData || []);
      const hData = await api.getSemesterTransitions();
      setHistory(hData || []);
    } catch {
      toast.error('Failed to load transition history');
    }
  };
  ...
  const executeTransition = async () => {
    const nextSem = parseInt(currentSem, 10) + 1;
    if (nextSem > 8) {
    ...
    setLoading(true);
    try {
      await api.promoteSemesterBatch({
        branch: selectedBranch,
        old_sem: parseInt(currentSem, 10),
        new_sem: nextSem,
      });

      await api.createSemesterTransition({
        branch: selectedBranch,
        old_sem: parseInt(currentSem, 10),
        new_sem: nextSem,
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
                ...
                <div className="flex items-center justify-between">
                   <div>
                     ...
                     <p className="text-xs text-slate-500">
                       Moving students of <span className="font-bold text-slate-900">{selectedBranch}</span> from <span className="font-bold text-slate-900">Sem {currentSem}</span> to <span className="font-bold text-indigo-600 underline">Sem {parseInt(currentSem, 10)+1}</span>
                     </p>
                   </div>
      ...
      <ConfirmModal 
        ...
        message={`Are you absolutely sure? This will promote ${students.length} students to Semester ${parseInt(currentSem, 10)+1}. This change is immediate and will be reflected in all student dashboards.`}
