...
export const BulkUpload = () => {
    ...
    try {
      ...
      for (const row of validRows) {
        ...
        const { data: newUser, error: userError } = await supabase.from('users').upsert({
          ...
          initial_password: row.initial_password || row.college_id // fallback to college_id if no password provided
        }, { onConflict: 'college_id' }).select().single();

        if (userError) {
          failCount++;
          failedRows.push({ ...row, reason: userError.message });
          continue;
        }

        // 2. If student, create in students table
        if (row.role.toLowerCase() === 'student') {
          ...
          if (studentError) {
            failCount++;
            failedRows.push({ ...row, reason: studentError.message });
            continue;
          }
        }

        // 3. MANDATORY: Trigger Auth Sync for login capability
        const { error: syncError } = await supabase.rpc('sync_user_to_auth', { target_id: newUser.id });
        if (syncError) {
          // Log but don't count as complete failure if public tables worked
          console.error('Auth sync failed for:', row.college_id, syncError);
        }
  ...
  const downloadTemplate = () => {
    const headers = [['college_id', 'name', 'role', 'roll_no', 'branch', 'sem', 'initial_password']];
